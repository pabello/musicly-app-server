...
CREATE TABLE public.performed_dump (
	old_recording_id bigint NOT NULL,
	old_artist_id bigint NOT NULL
);
...
INSERT INTO public.performed_dump(old_recording_id, old_artist_id)
	SELECT recording_id, artist_id
	FROM public.musicly_performed
	JOIN public.artist_dump ON (musicly_performed.artist_id = artist_dump.old_id);
...
INSERT INTO public.recording_dump(title, length, old_id)
	...
	JOIN public.performed_dump ON (musicly_recording.id = performed_dump.old_recording_id)
	ORDER BY id;
...
INSERT INTO public.recording_artist_dump(title, length, stage_name)
	SELECT rd.title,
		(SELECT length
			FROM public.recording_dump
				JOIN public.performed_dump ON (recording_dump.old_id = performed_dump.old_recording_id)
				JOIN public.artist_dump ON (artist_dump.old_id = performed_dump.old_artist_id)
	...
	FROM public.recording_dump rd
		JOIN public.performed_dump pd ON rd.old_id = pd.old_recording_id
		JOIN public.artist_dump ad ON pd.old_artist_id = ad.old_id
